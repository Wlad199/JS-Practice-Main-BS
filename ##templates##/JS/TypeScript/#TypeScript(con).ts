...
? утверждение типа Type Assertion (оператор 'as')
...
? Дженерики
	Дженерики в применении к функциям — это механизм, позволяющий создать такие функции, 
		которые имеют одинаковую логику обработки для разных типов данных. 
	...
	Дженерики могут иметь ограничения. Тип, который передается в дженерик, должен реализовывать какой-то интерфейс. 
...
? Pick 
	Тип Pick необходим для фильтрации типа по заданным свойствам.
		Остаются только указанные. Указывать можно только существующие в целевом типе. 
