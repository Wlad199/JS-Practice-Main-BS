...
d		С этим флагом результат регулярного выражения помещается в массив, 
			который содержит дополнительную информацию о регулярном выражении, 
			например индексы начала и конца подстрок. 
...
str[str.length - 1] or str.at(-1)
Квадратные скобки всегда возвращают undefined для отрицательных индексов
charAt и at делают одно и то же, at более современный и работает с отрицательными значениями
...
Переменная хранит не «значение объекта», а «ссылку» (адрес в памяти) на это значение.
Копирование такой переменной или передача её в качестве аргумента функции копирует эту ссылку, а не сам объект.
...
Колличество новых элементов будет равно кол. старых эл. (изменить нельзя!!!)
...
filter в отличии от map не меняет структуру массива, а меняет колличество элементов!!!
filter() возвращает новый массив, при этом исходный массив никак не изменится.
Если ни один элемент не прошел - вернет пустой массив.
...
Главной особенностью reduce(), является наличие аккумулятора.
